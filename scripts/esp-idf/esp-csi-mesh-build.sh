...
# Creates the wifi-mesh sub-project by copying active_sta as a starting
# point (same CMakeLists.txt boilerplate, same relative include pattern
# reaching into the shared top-level _components/ dir). Only runs once --
# if wifi-mesh already exists, later re-runs of this script leave it in
# place and just let apply_toolkit_overlay refresh the files that matter.
create_wifi_mesh_project() {
    local repo_root="${1}"

    if [ -d "${repo_root}/wifi-mesh" ]; then
        echo "wifi-mesh project already exists, continuing"
        return 0
    fi

    if [ ! -d "${repo_root}/active_sta" ]; then
        err "Cannot create wifi-mesh: ${repo_root}/active_sta not found."
        exit 1
    fi

    echo "Creating wifi-mesh project from active_sta"
    cp -r "${repo_root}/active_sta" "${repo_root}/wifi-mesh"

    # active_sta/CMakeLists.txt has `project(active-sta)` -- rename so the
    # built project/binary is identified correctly, not as active-sta.
    sed -i.bak 's/project(active-sta)/project(wifi-mesh)/' "${repo_root}/wifi-mesh/CMakeLists.txt"
    rm -f "${repo_root}/wifi-mesh/CMakeLists.txt.bak"

    # Drop any stale build artifacts/sdkconfig copied along with active_sta --
    # these get regenerated by set-target/reconfigure below and shouldn't
    # carry over active_sta's target or config.
    rm -rf "${repo_root}/wifi-mesh/build" \
           "${repo_root}/wifi-mesh/sdkconfig" \
           "${repo_root}/wifi-mesh/sdkconfig.old"

    echo "wifi-mesh project created"
}
export -f create_wifi_mesh_project
...
main() {
    ...
    create_wifi_mesh_project "third_party/esp32-csi-toolkit"
